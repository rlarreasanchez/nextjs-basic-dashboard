...
export async function generateStaticParams() {
  ...
  return static151Pokemons.results.map((pokemon) => ({
    name: pokemon.name,
  }));
}
...
export async function generateMetadata({
  params,
}: PokemonByNamePageProps): Promise<Metadata> {
  try {
...
const getPokemon = async (name: string): Promise<Pokemon> => {
  const pokemon = await fetch(`https://pokeapi.co/api/v2/pokemon/${name}`, {
    ...
    next: { revalidate: 60 * 60 * 30 * 6 /* 6 meses*/ },
  })
    .then((resp) => resp.json())
    .catch(() => {
      notFound();
...
export default async function PokemonByNamePage({
  params,
}: PokemonByNamePageProps) {
